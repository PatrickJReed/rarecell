...
def test_frozen_without_review_raises():
    with pytest.raises(UnreviewedProfileError, match="human_reviewed"):
        TargetCellProfile(**_minimal_profile_kwargs(), frozen=True, human_reviewed=False)


def test_frozen_with_review_succeeds_and_has_hash():
    p = TargetCellProfile(**_minimal_profile_kwargs(), human_reviewed=True, reviewer="r@x")
