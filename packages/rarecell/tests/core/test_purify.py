...
def test_purify_never_adds_cells_and_preserves_var():
    ...
    out = subcluster_and_purify(a, _profile(), suspect_clusters=["0"], cluster_key="leiden")
    assert out.n_obs <= a.n_obs
    assert list(out.var_names) == list(a.var_names)


def test_purify_handles_single_cell_suspect_cluster():
    rng = np.random.default_rng(2)
    n = 60
    X = sparse.csr_matrix(rng.poisson(2, size=(n, 30)).astype(float))
    leiden = ["1"] + ["0"] * (n - 1)  # suspect cluster "1" has exactly one cell
    a = ad.AnnData(X=X, obs={"leiden": leiden, "sample_id": ["s1"] * n})
    a.var_names = [f"G{i}" for i in range(30)]
    sc.pp.normalize_total(a)
    sc.pp.log1p(a)
    out = subcluster_and_purify(a, _profile(), suspect_clusters=["1"], cluster_key="leiden")
    assert isinstance(out, ad.AnnData)
    assert out.n_obs <= a.n_obs
