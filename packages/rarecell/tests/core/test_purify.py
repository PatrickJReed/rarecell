...
def test_purify_disabled_returns_input_unchanged():
    ...
    profile = profile.model_copy(update={
        "purify": PurifyParams(enabled=False, high_resolution=2.0,
                               min_cluster_purity=0.5),
    })
    out = subcluster_and_purify(a, profile, suspect_clusters=["0"],
                                cluster_key="leiden")
    assert out.n_obs == a.n_obs
...
def test_purify_empty_suspects_returns_input_unchanged():
    ...
    out = subcluster_and_purify(a, _profile(), suspect_clusters=[],
                                cluster_key="leiden")
    assert out.n_obs == a.n_obs
