...
def characterize_isolated(isolated: ad.AnnData, cfg: CNSTaxonomyConfig, *, cache_dir: Path) -> None:
    ...
    try:
        ...
        return
    isolated.obs["reference_cluster"] = result.per_cell_labels.reindex(
        isolated.obs_names
    ).to_numpy()
...
class IsolateRunner:
    ...
    def _s2b_class_gate(self) -> None:
        bundle_cache = Path.home() / ".cache" / "rarecell" / "bundles"
        narrowed, prov = apply_cns_class_gate(
            self.adata, self.profile.cns_taxonomy, cache_dir=bundle_cache
        )
        if self.profile.cns_taxonomy.enabled and narrowed.n_obs == 0:
            raise IsolationAbortedError(
                "CNS class gate removed all cells "
                f"(target_node={self.profile.cns_taxonomy.target_node!r}). "
                "Check the target node, reference_release, and min_confidence."
            )
        self.adata = narrowed
    ...
    def run(self) -> IsolateResult:
        ...
        try:
            ...
            bundle_cache = Path.home() / ".cache" / "rarecell" / "bundles"
            characterize_isolated(isolated, self.profile.cns_taxonomy, cache_dir=bundle_cache)
